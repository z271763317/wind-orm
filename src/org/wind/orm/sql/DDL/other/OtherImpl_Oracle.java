...
/**
 * @描述 : DDL【other】接口实现类——Oracle
 * @版权 : 湖南省郴州市安仁县胡璐璐
 * @时间 : 2021年5月12日 15:52:19
 */
public class OtherImpl_Oracle extends OtherImpl {
	...
	public String getTableSQL(){
		...
		try{
			String sql="select dbms_metadata.get_ddl('TABLE','"+TableUtil.getTable(super.getObj().getClass()).toUpperCase()+"') from dual;";
			ps=con.prepareStatement(sql);
			rs=super.executeSQL(SQLType.SELECT,ps, sql.toString(), null);
			while(rs.next()){
				tableSQL=rs.getString(1);		//建表SQL
			}
		}catch (Exception e) {
